(ns rolls-royce-flight.main-test
  "Contract + behavioral test for the rolls_royce_flight-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "rolls_royce_flight-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'rolls-royce-flight.main-test)]
