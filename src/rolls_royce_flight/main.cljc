(ns rolls-royce-flight.main
  "Kotodama WASM entrypoint for the Rolls_royce_flight clean-room actor (L4) — Clojure port.
  ...
  facts (`rolls_royce_flight.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "rolls_royce_flight")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"rolls_royce_flight.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "rolls_royce_flight-compat" :tier tier :entities entities} 200])
...
(defn start! [] :rolls_royce_flight-compat/ready)
